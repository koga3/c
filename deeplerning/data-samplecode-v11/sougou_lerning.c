...
//配列のランダムシャフル
void swap(int *pa, int *pb) //入力二つの値を交換する
{
  int temp = *pa;
  *pa = *pb;
  *pb = temp;
...
//平均０、分散sqrt(2/n)のガウス分布乱数による初期化
double Uniform(void) //一様分布の生成
{
...
int main()
{
  ...
  //パラメータの初期化
  rand_normal_init(784 * 50, A1);
  rand_normal_init(50, b1);
  rand_normal_init(50 * 100, A2);
  rand_normal_init(100, b2);
  rand_normal_init(100 * 10, A3);
  rand_normal_init(10, b3);
  ...
  do
  {
    ...
    correst_rate_test = (float)(sum * 100.0 / test_count);//テストデータに対する正解率
    printf("  %f%%\n\n", correst_rate_test);
    //taining
    int sum1 = 0; //正解数
    for (int i = 0; i < train_count; i++)
    ...
    correst_rate_train = (float)(sum1 * 100.0 / train_count);//訓練データに対する正解率
    k++;
    //エラーメッセージ
    if (correst_rate_train - correst_rate_train > (float)10.0)
      printf("Over Lerning!!");
  } while ((k < epoc_count) && (correst_rate_train - correst_rate_train < (float)10.0));//過学習の防止
